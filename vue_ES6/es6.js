...
function test_1() {
  let x = "let";
  {
    ...
    // x = "changed"; //상수 (constant)는  값의 변경을 할 수 없으므로 에러
  }
  // x 출력
  if (x === "let") {
    console.log("I am " + x);
...
// 2. template string  ==> 변수와 데이터에 대한 '+' 접합연산을 최소화,  멀티라인 지원
function test_2() {
  const id = "IronMan";
  const password = "1234";
  const name = "토니 스타크";
  ...
  console.log("userString의 타입>>>"+ typeof(userString));
...
function test_3() {
  this.a = 10;
  this.b = 20;
  // let max = function( ){  return (a > b ? a : b);   };
  let max = () => (a > b ? a : b);
  console.log(max());
